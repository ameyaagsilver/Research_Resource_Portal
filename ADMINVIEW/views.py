...
from LOGIN.views import get_user_instance_by_id
from RESEARCH_RESOURCE_PORTAL.settings import BASE_DIR, MEDIA_ROOT
...
def issueResource(request):
    ...
    if request.method == "POST" and isAdmin and request.POST.get('emailID'):
        ...
        if resource.quantity >= 1:
            ...
            # Threading for sending a mail to a user
            print("**********SENDING RESOURCE ISSUE CONFIRMATION THROUGH MAIL TO USER**********")
...
def returnResource(request):
    ...
    if request.method == "POST" and isAdmin and request.POST.get('logID'):
        ...
        borrowed_user = get_user_instance_by_id(resource_logbook_record.member_id_id)
        # print("!@#$%^&*")
        # print(resource_logbook_record)
        # print(resource_logbook_record.member_id_id)
        # print("!@#$%^&*")
        # print("USER ID = == = ==", resource_logbook_record.member_id)
        # print(borrowed_user)
        # Threading for sending a mail to a user
        print("**********SENDING RESOURCE RETURN CONFIRMATION THROUGH MAIL TO USER**********")
        threadAddResource = Thread(target=sendReturnConfirmationThroughMail, args=('rvce.resource.portal@gmail.com', borrowed_user, resource, admin, resource_logbook_record))
        threadAddResource.start()
        print("*********************")
        # sendIssueConfirmationThroughMail('rvce.resource.portal@gmail.com', user, resource, admin, new_logbook_instance)
...
def sendIssueConfirmationThroughMail(From, user, resource, admin, new_logbook_instance):
    # sleep(1)
    print(user, resource, new_logbook_instance, admin, "STARTING INFO####")
...
<table align="center" cellpadding="0" cellspacing="0" width="100%" height="100%">
  <tr>
            ...
            <table cellpadding="0" cellspacing="0" width="600" class="w320">
              <tr>
                <td align="left" class="mobile-padding" style="padding:20px">
                  ...
                  <div>
                    Issued to: <b>""" + str(user.first_name) + " " + str(user.middle_name) + " " + str(user.last_name) +"""</b><br>
                    Issued by: <b>""" + str(admin.admin_first_name) + " " + str(admin.admin_middle_name)+ " " + str(admin.admin_last_name) +"""(System admin, """ + str(admin.admin_department_name) + """)</b><br>
                    Issue date: <b>""" + str(new_logbook_instance.issue_date) + """</b><br>
                    Email-id: """ + str(user.emailID) + """
                    <br>
                    ...
                    Resource name: <strong>""" + str(resource.resource_name) + """</strong><br>

                    Resource OEM: <strong>""" + str(resource.OEM)+ """</strong><br><br><br>

                    <strong>About the Resource: </strong>""" + str(resource.about) + """<br><br><br>

                    <strong>NOTE:</strong> Kindly return it after the use to: <strong>""" + str(resource.location) + ", " + str(resource.department_name) + """</strong><br>
                  </div>
    ...
    mimeMessage['to'] = user.emailID
    mimeMessage['subject'] = f'Resource ISSUED confirmation (Issue ID: {str(new_logbook_instance.log_id)})'
    mimeMessage.attach(MIMEText(emailMsg, 'html'))
    
    fileName = MEDIA_ROOT +"\\"+ str(resource.image).replace("/", "\\")
...
def sendReturnConfirmationThroughMail(From, user, resource, admin, old_logbook_instance):
    # sleep(1)
    print("INSIDE THE send confirmation through mail THREAD...")
    print("**************************8", From, user, resource, admin, old_logbook_instance)
    emailMsg = str("""\n
    <!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">
<html xmlns="http://www.w3.org/1999/xhtml">
<head>
              ...
              <tr>
                <td align="left" class="mobile-padding" style="padding:20px">
                  ...
                  <div>
                    Issued to: <b>""" + str(user.first_name) + " " + str(user.middle_name) + " " + str(user.last_name) +"""</b><br>
                    Issued by: <b>""" + str(admin.admin_first_name) + " " + str(admin.admin_middle_name)+ " " + str(admin.admin_last_name) +"""(System admin, """ + str(admin.admin_department_name) + """)</b><br>
                    Issue date: <b>""" + str(old_logbook_instance.issue_date) + """</b><br>
                    Return date: <b>""" + str(old_logbook_instance.return_date) + """</b><br>
                    User Email-id: """ + str(user.emailID) + """
                    <br>
                    ...
                    <strong>About the Resource: </strong>""" + str(resource.about[:200]) + """...<br><br><br>

                    <strong> """ + str(user.first_name) + """, you have successfully returned the above mentioned resource to concerned authority.</strong><br>

                    <br>
                    <br>
                    <br>
                    Thanks,<br>
                    Team <strong>Research Resource Portal</strong>
                  </div>
...
<!-- 
    ...
    mimeMessage = MIMEMultipart()
    mimeMessage['to'] = user.emailID
    mimeMessage['subject'] = f'Resource RETURNED confirmation (Issue ID: {str(old_logbook_instance.log_id)})'
    mimeMessage.attach(MIMEText(emailMsg, 'html'))
    fileName = MEDIA_ROOT +"\\"+ str(resource.image).replace("/", "\\")
    with open(fileName, "rb") as fp:
        (content_type, encoding) = mimetypes.guess_type(fileName)
...
def get_resource_instance_by_id(resourceID):
    resource = userviewMODELS.resources.objects.filter(resource_id=resourceID).first()
    return resource
...
#     print(user, resource, new_logbook_instance, admin)
...
#     # print("EMAIL_PASSWORD", EMAIL_PASSWORD)
#     print("**************************8", From, user, resource, admin, old_logbook_instance)
#     Subject = f'Resource RETURNED confirmation (Issue ID: {str(old_logbook_instance.log_id)})'
#     mssg = EmailMessage()
#     mssg['Subject'] = Subject
#     mssg['From'] = From
#     mssg['To'] = user.emailID
